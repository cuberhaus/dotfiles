...
# Auto-discover everything under .github/skills/ and .github/hooks/
# (SKILL.md, references, hook configs and scripts).
for subdir in .github/skills .github/hooks; do
    [[ -d "${source_dir}/${subdir}" ]] || continue
    while IFS= read -r -d '' f; do
        rel="${f#${source_dir}/}"
        pairs+=("${rel}|${rel}")
    done < <(find "${source_dir}/${subdir}" -type f -print0 | sort -z)
done
